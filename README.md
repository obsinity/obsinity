...
- **Multi-attribute state snapshots & transitions**: `StateDetectionService` + `/api/query/state-counts` (current distribution) and `/api/query/state-transitions` (A→B flows) turn arbitrary attribute changes into real-time counters.
