...
public final class OBJqlCteBuilder {
	...
	public Built build(OBJql q, OBJqlPage page) {
		...
		sql.append("WITH base AS (\n")
			...
			.append("  FROM ").append(eventsTable).append(" e\n")
		...
		for (int i = 0; i < attrCtes.size(); i++) {
			if (i == 0) {
				...
				sql.append("  INTERSECT SELECT event_id FROM ").append(attrCtes.get(i)).append("\n");
			}
		}
		sql.append("),\n");
		...
		String order = switch (orderField.toLowerCase(Locale.ROOT)) {
		...
		sql.append("SELECT e.*,(SELECT matched_count FROM counts) AS matched_count\n")
			...
			.append("ORDER BY ").append(order.replace("b.", "e.")).append("\n");
	...
	private void appendAttrCte(StringBuilder sql, Map<String,Object> p, String cte, String path, OBJql.Predicate pred) {
		...
		if (pred instanceof OBJql.Eq eq) {
			Object rhs = eq.rhs();
			sql.append("    AND (x.value_text = :").append(pname)
				.append(" OR x.value_numeric = :").append(pname)
				.append(" OR x.value_bool = :").append(pname).append(")\n");
			p.put(pname, rhs);
		} else if (pred instanceof OBJql.Ne ne) {
			Object rhs = ne.rhs();
			sql.append("    AND NOT (x.value_text = :").append(pname)
				.append(" OR x.value_numeric = :").append(pname)
				.append(" OR x.value_bool = :").append(pname).append(")\n");
			p.put(pname, rhs);
			...
			throw new IllegalArgumentException("Unsupported attr predicate: " + pred.getClass().getSimpleName());
		}
		sql.append(")\n");
	}
	...
	private void appendEnvelopePredicate(StringBuilder sql, Map<String,Object> p, String name, OBJql.Predicate pred) {
		...
		if (pred instanceof OBJql.Eq eq) {
			...
			throw new IllegalArgumentException("Unsupported envelope predicate: " + pred.getClass().getSimpleName());
		}
	}
