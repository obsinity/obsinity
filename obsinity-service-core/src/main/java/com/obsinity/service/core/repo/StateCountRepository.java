...
@Repository
public class StateCountRepository {

    private final NamedParameterJdbcTemplate jdbc;

    public StateCountRepository(NamedParameterJdbcTemplate jdbc) {
        this.jdbc = jdbc;
    }
    ...
    private void updateCount(UUID serviceId, String objectType, String attribute, String stateValue, long delta) {
        ...
        jdbc.update(
                """
            insert into obsinity.state_counts(service_id, object_type, attribute, state_value, count)
            values (:service_id, :object_type, :attribute, :state_value, :delta)
            on conflict (service_id, object_type, attribute, state_value)
            do update set count = greatest(0, obsinity.state_counts.count + :delta)
            """,
                params);
    }
}
