...
public final class OBJqlParser {
	...
	private static final Pattern KV = Pattern.compile("(service|event)\\s*[:=]\\s*(\"[^\"]+\"|\\S+)", Pattern.CASE_INSENSITIVE);
	private static final Pattern SELECT = Pattern.compile("\\bselect\\s+(.+?)\\s+(where|since|between|order|limit|$)", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
	private static final Pattern WHERE = Pattern.compile("\\bwhere\\b(.+?)(since|between|order|limit|$)", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);
	private static final Pattern ORDER = Pattern.compile("\\border\\s+by\\s+(\\S+)\\s*(asc|desc)?", Pattern.CASE_INSENSITIVE);
	private static final Pattern LIMIT = Pattern.compile("\\blimit\\s+(\\d+)", Pattern.CASE_INSENSITIVE);
	private static final Pattern BETWEEN = Pattern.compile("\\bbetween\\s+(\\S+)\\s*\\.\\.\\s*(\\S+)", Pattern.CASE_INSENSITIVE);
