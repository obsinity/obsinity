...
| **Rollup Strategy**    | **Fixed rollups (5s, 1m, 1h, 1d, 7d)**, materialized at ingest             | Continuous rollups             | Retention + downsampling           | Rollups at query time | Relies on ClickHouse           | Partition pruning only | Predictable cost & latency: queries over 7 days never explode in runtime rollup.                              |
...
| **Query Semantics**    | Row-oriented: `(time bucket × dimension tuple)`; compressed row format     | SQL rollups                    | Time-series only, PromQL functions | Arbitrary OLAP             | Rolled Up metrics/logs/traces | SQL rollups         | Obsinity’s results align with how observability data is consumed (dashboards, SLOs).                               |
...
2. **Predictable Performance:**
    ...
    * Avoids the “query-time blowup” problem that plagues Timescale continuous rollups or ClickHouse ad-hoc queries.
