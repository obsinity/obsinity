...
| **Query Language**           | OB-SQL / OB-JQL (SQL-like + JSON) hides PostgreSQL; supports rich filtering, rollup, arbitrary intervals. Always consistent with OTEL semantics.     | InfluxQL (SQL-like) and Flux (functional). Strong for time-bucketed queries, weaker for relational operations or complex filters.          |
