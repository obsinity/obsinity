...
- Idempotency is enforced using a required event ID and SHA‑256 body hash; mismatches route to a DLQ.
...
- Each ingest computes a SHA‑256 hash of the canonicalized body and compares to any existing event with the same ID:
  ...
  - If ID exists and hash differs: route to DLQ with diagnostics (conflicting payload).
...
- Pattern
  ...
  - Dispatcher polls, calls connector, marks delivered; retries with backoff and DLQ on repeated failure.
- Targets
  - Kafka: topic per service/domain, keys by event_id or attribute; include `traceparent` header.
  - RabbitMQ: exchange/key templates, persistent delivery, DLQ and retry policy.
...
- Configuration
  ...
  - Global: batching, concurrency, backoff, DLQ.
...
- Metrics
  - Ingest: requests/sec, success/duplicate/DLQ counts, latency, percentiles.
  - Storage: partition counts, bloat/vacuum metrics, query timing.
  - Connectors: processed/sec, retries, DLQ.
...
## 11) Roadmap (Phased)
- Phase A: finalize canonical payload + idempotency/DLQ; REST + Postgres; search; partition maintenance.
...
2) Stream Ingest (Kafka/Rabbit)
   - Consumer poll → Parse message to canonical JSON → Map to EventEnvelope → Write DB → Commit offset/ACK → DLQ on fatal
3) Forwarding
   - Dispatcher load batch from outbox → Connector send → Mark delivered or backoff; on repeated failure put to DLQ
...
- Broker outages: backpressure via consumer backoff; connectors buffer with exponential backoff; DLQ capture.
...
- Evidence: dashboards (ingest rates, DLQ counts), backup reports, restore drills.
...
- ADR‑002 Idempotency by (event_id + SHA‑256 body) with DLQ on mismatch — decided.
